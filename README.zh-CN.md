...
与传统 PDF 转 Markdown 工具（如 Microsoft MarkItDown、PyMuPDF 文本提取等）相比，PDF2MD 采用**多模态图像识别方案**：

| 特性 | PDF2MD | 传统工具 (MarkItDown, PyMuPDF 等) |
|---|---|---|
...
## 与 Microsoft MarkItDown 的对比

[Microsoft MarkItDown](https://github.com/microsoft/markitdown) 是一个优秀的通用文档转换工具，但在 PDF 处理方面有明显局限：

| 维度 | PDF2MD | MarkItDown |
|---|---|---|
| **方法论** | 多模态视觉识别（看图说话） | 文本流提取 (PyMuPDF) |
| **公式支持** | 完整 LaTeX，含验证 | 无公式处理 |
| **扫描件** | 原生支持 | 依赖 OCR，效果有限 |
| **乱码问题** | 不存在（读取图像） | 自定义字体常导致乱码 |
| **表格** | 忠实转录 | 简单提取，易损坏 |
| **图表** | 可裁剪为图像 | 通常丢失 |
| **验证** | 5 阶段验证流水线 | 无验证 |
| **速度** | 较慢（LLM 逐页处理） | 快（纯程序化） |
| **适用场景** | 学术论文、教材、公式密集文档 | 通用文档批量转换 |

**总结**：MarkItDown 适合快速批量转换结构简单的文档；PDF2MD 适合需要高精度转换的学术文档，尤其是包含大量公式、图表的 PDF。
